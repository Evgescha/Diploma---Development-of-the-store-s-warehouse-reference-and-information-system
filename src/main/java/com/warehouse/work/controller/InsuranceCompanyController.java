...
public class InsuranceCompanyController {
    ...
    String get(Model model) {
        List<InsuranceCompany> list = service.repository.findAll();
    ...
    public String edit(Model model, @PathVariable(name = "id", required = false) Long id) {
        if (id != null) {
            InsuranceCompany entity = service.read(id);
            model.addAttribute("entity", entity);
        } else {
            model.addAttribute("entity", new InsuranceCompany());
        }
        return "insuranceCompany-add-edit";
    }

    @RequestMapping(path = "/create", method = RequestMethod.POST)
    public String createOrUpdate(InsuranceCompany entity) throws Exception {
